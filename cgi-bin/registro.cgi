...
if($password ne $Rpassword){
        ...
        if($username =~ m[^[a-z_]([a-z0-9_-]{0,31}|[a-z0-9_-]{0,30}\$)$])
        {
                ...
                if(!$chk_id){
                        ...
                        my $q = $db->prepare("insert into avanzadb.users(id,password,name,surname,address,phone,email,local_email,role) values (?,?,?,?,?,?,?,?,?)");
                        $q->execute($username,$encode,$name,$surname,$address,$phone,$email,$local_email,$role);
                        $q->finish();

                        my $body = ("<p>Hola, solo queda un paso para disfrutar de los servicios de AvanzaII</p> <a href='https://avanzaii/activacion.html?user=$username'> Pulsa aqui e introduzca sus credenciales</a>");
